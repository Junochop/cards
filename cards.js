...
//add an EventListener feature to the "create" button.
...
//declare DOM string function
function dom(){
  var newString = '';
  
  for (var i = 0; i < card.length; i++) {
  newString += `<div class ="newCard-${i}" class="newCards" id="nCard-${i}">`;
  newString += `<h1>` + card[i].name +`</h1>`;
  newString += `<button type="input" class="deButt" id="dButton-${i}">delete</button>`
  newString += `</div>`;
  }

  writeToDom(newString); 

}

//declare writeToDom function.
function writeToDom(x) {
	
	document.getElementById('addedCard').innerHTML = x;
    
}

//declare a function of adding card values to an empty array if the input field is not empty.
function writeToArray() {
	if (newCard.value != ""){
	card.push({name: newCard.value});
	console.log(card);
	}
}
...
function deleteCard() {
  for(var i = 0; i < card.length; i++){
  var deleteButton = document.getElementById(`dButton-${i}`);
  var deleteTargetId = document.getElementById('addedCard');
  deleteButton.addEventListener('click', function() {
  event.target.parentNode.remove();
  //event.target.remove(deleteTargetId);
  console.log('hi');
  });
 } 
}
...
// function deleteCard() {
// for (var i = 0; i < card.length; card++)	{
//   var deleteButton = document.getElementById(`dButton-${i}`);
//   //var deleteTargetId = document.getElementById(`nCard-${j}`);
